...
// // Interface for the uploaded file with preview
...
//       // Load order images if exist
//       if (order?.images && order?.images.length > 0) {
//         this.loadOrderImages(order?.images);
//       }
...
//   loadOrderImages(images: any[]): void {
//     // Implementation depends on how images are stored
//     // Example for base64 images:
//     images.forEach(imageData => {
//       const preview = imageData.url || imageData.path || imageData.data;
//       this.selectedFiles.push({
//         file: null as any, // You won't have the actual file object when loading from server
//         name: imageData.name || 'תמונה',
//         preview: preview
//       });
//     });
//   }
...
//     itemGroup.patchValue({
//       serviceId: service.id,
//       price: service.price // Now the price is just a suggestion and can be changed by the user
//     });
...
//       if (fileWithPreview.file) { // Only append if we have the actual file (not for existing images in edit mode)
//         formData.append(`images`, fileWithPreview.file, fileWithPreview.name);
//       }
//     });
    
//     if (this.isEditMode) {
//       // Add image metadata for existing images (no file)
//       const existingImages = this.selectedFiles
//         .filter(f => !f.file)
//         .map(f => ({ name: f.name, preview: f.preview }));
        
//       formData.append('existingImages', JSON.stringify(existingImages));
      
//       this.orderService.updateOrderWithImages(this.orderId, formData).subscribe(
//         () => {
//           this.snackBar.open('ההזמנה עודכנה בהצלחה', 'סגור', {
//             duration: 3000,
//             direction: 'rtl'
...
//       );
//     } else {
//       this.orderService.createOrderWithImages(formData).subscribe(
//         (result: Order) => {
//           this.snackBar.open('ההזמנה נוצרה בהצלחה', 'סגור', {
//             duration: 3000,
//             direction: 'rtl'
